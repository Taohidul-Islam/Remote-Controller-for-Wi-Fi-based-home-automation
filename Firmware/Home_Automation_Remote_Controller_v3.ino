 ...
 *   - * + 0 + 9 TRUE simultaneously: factory reset main board
 *   - * + 0 + 1 TRUE simultaneously: OTA update main board
 *   - # + 0 + 1 TRUE simultaneously: OTA update this remote
 *
 * Combo Detection:
 *   Full keypad matrix scanned every loop to detect ALL keys held at once.
 *   Combo fires when all 3 required keys are down simultaneously and then
 *   all released (fire-on-release to avoid accidental repeat).
 *   * 0 9 = factory reset main board
 *   * 0 1 = OTA update main board
...
// ─── Full Matrix Scan (all keys at once) ─────────────────────
// Returns a bitmask of all currently pressed keys.
// Bit index = r*COLS + c
uint16_t scanAllKeys() {
  uint16_t mask = 0;
...
// Convert a key character to its bitmask bit position
int keyToBit(char k) {
  for(int r=0;r<ROWS;r++)
    for(int c=0;c<COLS;c++)
      if(keyMap[r][c]==k) return r*COLS+c;
  return -1;
}

bool keyInMask(uint16_t mask, char k) {
  int b=keyToBit(k);
  return (b>=0) && (mask & (1<<b));
}

// ─── Single Key Detection (debounced, one-shot) ──────────────
char     lastSingleKey   = 0;
uint16_t lastMask        = 0;

// Returns a single key on first press (edge detect), 0 otherwise.
// Only fires when exactly 1 key is held (ignores multi-key presses).
char getKey() {
  uint16_t mask = scanAllKeys();
  // Count pressed keys
  int count = 0;
  char foundKey = 0;
  for(int r=0;r<ROWS;r++)
    for(int c=0;c<COLS;c++)
      if(mask & (1<<(r*COLS+c))) { count++; foundKey=keyMap[r][c]; }

  if(count==1) {
    if(foundKey != lastSingleKey) {
      lastSingleKey = foundKey;
      lastMask = mask;
      return foundKey;
    }
  } else {
...
// ─── Simultaneous Combo Detection ────────────────────────────
// Combos fire when ALL required keys are held at the same time,
// then released. Fire-on-release prevents re-triggering.
//
//  1 = * + 0 + 9 → factory reset main board
//  2 = * + 0 + 1 → OTA update main board
//  3 = # + 0 + 1 → OTA update this remote

bool comboFired     = false;  // true while combo keys are held (already fired)
int  pendingCombo   = 0;      // which combo was detected

int checkSimultaneousCombo(uint16_t mask) {
  bool star  = keyInMask(mask,'*');
  bool hash  = keyInMask(mask,'#');
  bool zero  = keyInMask(mask,'0');
  bool one   = keyInMask(mask,'1');
  bool nine  = keyInMask(mask,'9');

  if(star && zero && nine)  return 1;  // * 0 9 → reset
  if(star && zero && one)   return 2;  // * 0 1 → HA OTA
  if(hash && zero && one)   return 3;  // # 0 1 → remote OTA
  return 0;
}

// Call every loop. Returns combo ID on release, 0 otherwise.
int updateCombo() {
  uint16_t mask = lastMask;  // reuse what getKey() already scanned
  int combo = checkSimultaneousCombo(mask);
  ...
  if(combo != 0) {
    ...
    return 0;  // still held, don't fire yet
  } else {
    // All combo keys released
    if(comboFired) {
      comboFired = false;
      int fired  = pendingCombo;
      pendingCombo = 0;
      return fired;  // fire on release
    }
  }
  return 0;
}
...
void handleKey(char k) {
  ...
  switch(k) {
    ...
    // *, #, 1, 0, 9 — part of simultaneous combos, no standalone action
    case '*': break;
    case '#': break;
...
void loop() {
  ...
  // Single key (only fires when exactly 1 key held — also updates lastMask)
  char k = getKey();

  // Simultaneous combo check (uses lastMask populated by getKey)
  int combo = updateCombo();
  if(combo != 0) {
    // Suppress any single-key action when a combo fires
    k = 0;
    switch(combo) {
      case 1:
        // * + 0 + 9 → factory reset main board
        Serial.println("[COMBO] * 0 9 → Factory reset main board");
        rapidBlink(5);
      ...
      case 2:
        // * + 0 + 1 → OTA update main board
        Serial.println("[COMBO] * 0 1 → OTA update main board");
        otaUpdateMainBoard();
        break;
      case 3:
        // # + 0 + 1 → OTA update this remote
        Serial.println("[COMBO] # 0 1 → OTA update remote");
        otaUpdateRemote();
        break;
    }
  }
